...
export default function Reception() {
  ...
  const [clinicId, setClinicId] = useState(''); // <-- nuevo estado
  ...
  async function loadRecent() {
    setLoading(true);
    try {
      const { data } = await api.get('/tickets/recent?limit=8');
      setRecent(data);
    } catch (e) {
      console.error(e);
    } finally { 
  ...
  useEffect(() => {
    ...
    socketRef.current.on('queue:update', loadRecent);
    return () => {
      if (socketRef.current) socketRef.current.off('queue:update', loadRecent);
    };
  }, []);
  ...
  async function submit(e) {
    ...
    try {
      const body = { 
        patient: { name, document: doc, phone },
        clinic_id: clinicId ? parseInt(clinicId) : null, // <-- incluir clinic_id
      };
    ...
    <div className="grid md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2">
        <Card title="Últimos tickets">
          {loading ? (
            <div className="flex items-center gap-2"><Spinner/>Cargando...</div>
          ) : (
            <ul className="space-y-2">
              {recent.length === 0 && (
                <div className="text-sm text-slate-500">Sin tickets recientes</div>
              )}
